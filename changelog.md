...
This changelog refers to the differences / changes between the [first site (previous version)](https://github.com/glmvc/first-site) and the upgraded version of it here.
...
- [x] Linked to the new [normalize.css](https://necolas.github.io/normalize.css/) file:
...
- [x] Two `div` container added after the SVG logo for text with a typewriter effect

- [x] Navigation with hyperlinks to the other HTML documents (code.html, changelog.html and "old" index.html) added withing the header:

  `<nav>...</nav>`

- [x] One `div` container for the mobile navigation icon added with nine `span` elements within
...
- [x] Figure and figure caption added to the image, video and audio:
...
- [x] Addditional website information added within a detail area with a summary text:

  `<details>...</details>` with `<summary>Info</summary>` within

- [x] Superscript added for the birthdate of the website within the detail area in the footer:
...
- [x] Last update text made as a small side comment within the detail area in the footer:
...
- [x] External stylesheets, internal stylesheet and inline styling added (internal and inline styling just to show it off)
...
- [x] Standard box model (content-box) changed to the alternative box model for all elements in the document:
...
- [x] Cursor styled accordingly and as needed:
...
- [x] Main heading (`<h1>`) wrapped in a flex container to center it with an inner display type:
...
- [x] Caption of the table positioned to the bottom:

  `table caption {caption-side: bottom;}`

- [x] Adjusted how word breaking / wrapping and whitespace is treated:
...
- [x] The website is styled to be printable:
...
- [x] [jQuery library](https://jquery.com/) now in use and most code rewritten using its syntax
